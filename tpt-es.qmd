...
capture_output <- capture.output({
  shp <- st_read("D:/##AYUPA DESKTOP 2/SPATIAL/indonesia-prov.geojson")
})
shp$kode <- as.character(shp$kode)

# Group by 'prov', 'isic', and 'year' and count the number of establishments
num_by_prov <- tpt_combine_est %>%
  group_by(prov, isic2, year) %>%
  summarise(num_est = n()) %>%
  ungroup() %>%
  pivot_wider(names_from = isic2, values_from = num_est, values_fill = list(num_est = 0)) %>%
  rename(tekstil = `13`) %>%
  rename(pakaian = `14`) %>%
  mutate(total_tpt = tekstil + pakaian)

# Group by year
num_by_year <- num_by_prov %>%
  group_by(year) %>%
  group_split()

# Data 2023
shp_2023 <- shp %>% 
  left_join(num_by_year[[8]], by = c("kode" = "prov")) %>%
  mutate(tekstil    = ifelse(tekstil    == 0, NA, tekstil)) %>%
  mutate(pakaian    = ifelse(pakaian    == 0, NA, pakaian)) %>%
  mutate(total_tpt  = ifelse(total_tpt  == 0, NA, total_tpt))


# Add info label
Statistik2023 <- paste0(
  "\nProvinsi: ", shp_2023$Propinsi,
  "\nJumlah industri TPT: ", shp_2023$total_tpt,
  "\nJumlah industri tekstil: ", shp_2023$tekstil,
  "\nJumlah industri pakaian jadi: ", shp_2023$pakaian
)
shp_2023$Statistik2023 <- Statistik2023
...
## Arahkan kursor ke peta untuk melihat rincian statistik.
:::

```{r, warning=FALSE}
# Plot map
plottpt_23 <- ggplot(shp_2023) +
  geom_sf(aes(fill = total_tpt, label = Statistik2023)) +
  scale_fill_viridis_c(limits = range(c(1, 2380)), name = "") +   # same range as Java
  ggtitle("Sebaran Industri Tekstil dan Produk Tekstil") +
  theme_minimal()

ggplotly(plottpt_23, tooltip = "label")%>%
  layout(
    annotations = list(
      x = 1, y = -0.09,
      xref = "paper", yref = "paper",
      text = "Sumber: Updating DPA (2023)",
      showarrow = FALSE,
      font = list(size = 10)
    ))
...
### Tren industri tekstil dan pakaian jadi di Pulau Jawa, 2019-2023

```{r, warning=FALSE}

# Data 2019
shp_2019_java <- shp %>% 
  left_join(num_by_year[[4]], by = c("kode" = "prov")) %>%
  mutate(tekstil    = ifelse(tekstil    == 0, NA, tekstil)) %>%
  mutate(pakaian    = ifelse(pakaian    == 0, NA, pakaian)) %>%
  mutate(total_tpt  = ifelse(total_tpt  == 0, NA, total_tpt)) %>%
  filter(kode > 30 & kode <40)

# Add info label
Statistik2019 <- paste0(
  "\nProvinsi: ", shp_2019_java$Propinsi,
  "\nJumlah industri TPT: ", shp_2019_java$total_tpt,
  "\nJumlah industri tekstil: ", shp_2019_java$tekstil,
  "\nJumlah industri pakaian jadi: ", shp_2019_java$pakaian
)
shp_2019_java$Statistik2016 <- Statistik2019
...
# Add info label
Statistik2023 <- paste0(
  "\nProvinsi: ", shp_2023_java$Propinsi,
  "\nJumlah industri TPT: ", shp_2023_java$total_tpt,
  "\nJumlah industri tekstil: ", shp_2023_java$tekstil,
  "\nJumlah industri pakaian jadi: ", shp_2023_java$pakaian
)
shp_2023_java$Statistik2023 <- Statistik2023



plottpt_19_java <- ggplot(shp_2019_java) +
  geom_sf(aes(fill = total_tpt, label = Statistik2019)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2019_java$total_tpt)), name = "") +
#  ggtitle("Sebaran Industri TPT 2016") +
  theme_void()

plottpt_23_java <- ggplot(shp_2023_java) +
  geom_sf(aes(fill = total_tpt, label = Statistik2023)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2019_java$total_tpt)), name = "") +
  ggtitle("Sebaran Industri TPT 2019 (atas) dan 2023 (bawah)") +
  theme_void()


combined_plot_java <- subplot(
  ggplotly(plottpt_19_java, tooltip = "label") %>%
    layout(
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
      text = "Sumber: STPIM 2019 (atas), Updating DPA 2023 (bawah)",
      showarrow = FALSE,
      font = list(size = 10)
      )),
  ggplotly(plottpt_23_java, tooltip = "label") %>%
    layout(
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
#      text = "Sumber: S STPIM 2019, Updating DPA 2023",
      showarrow = FALSE,
      font = list(size = 10)
      )),
  nrows = 2)
...
tpt32_19 <- format(shp_2019_java$total_tpt[shp_2019_java$kode == 32], big.mark = ".")
tpt32_23 <- format(shp_2023_java$total_tpt[shp_2023_java$kode == 32], big.mark = ".")

```

Isu relokasi perusahaan TPT dari suatu wilayah ke wilayah lain di Pulau Jawa dikarenakan faktor kenaikan upah minimum tidak terlihat pada peta tren sebaran industri di atas. Provinsi Jawa Barat, contohnya, mengalami penurunan jumlah industri TPT sepanjang 2019 hingga 2023, dari `r tpt32_19` menjadi `r tpt32_23` perusahaan. Penurunan jumlah industri di Provinsi Jawa Barat tidak menunjukkan indikasi adanya relokasi usaha ke provinsi lain dengan upah minimum yang lebih rendah, seperti Provinsi Jawa Tengah, sebab semua provinsi di Pulau Jawa, kecuali DI Yogyakarta dan Banten, tercatat mengalami penurunan jumlah perusahaan industri TPT sepanjang 2016-2023.

```{r, warning=FALSE, include=FALSE}
# Tekstil
plottpt_19_java13 <- ggplot(shp_2019_java) +
  geom_sf(aes(fill = tekstil, label = Statistik2019)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2019_java$total_tpt)), name = "") +
#  ggtitle("Sebaran Industri TPT 2019") +
  theme_void()

plottpt_23_java13 <- ggplot(shp_2023_java) +
  geom_sf(aes(fill = tekstil, label = Statistik2023)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2019_java$total_tpt)), name = "") +
  ggtitle("Sebaran Industri Tekstil 2019 (atas) dan 2023 (bawah)") +
  theme_void()


combined_plot_java13 <- subplot(
  ggplotly(plottpt_19_java13, tooltip = "label") %>%
    layout(
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
  ...
  ggplotly(plottpt_23_java13, tooltip = "label") %>%
    layout(
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
#      text = "Sumber: STPIM 2019, Updating DPA 2023",
      showarrow = FALSE,
      font = list(size = 10)
      )),
  nrows = 2)
combined_plot_java13




# Pakaian Jadi
plottpt_19_java14 <- ggplot(shp_2019_java) +
  geom_sf(aes(fill = pakaian, label = Statistik2019)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2019_java$total_tpt)), name = "") +
#  ggtitle("Sebaran Industri TPT 2019") +
  theme_void()

plottpt_23_java14 <- ggplot(shp_2023_java) +
  geom_sf(aes(fill = pakaian, label = Statistik2023)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2019_java$total_tpt)), name = "") +
  ggtitle("Sebaran Industri Pakaian Jadi 2019 (atas) dan 2023 (bawah)") +
  theme_void()


combined_plot_java14 <- subplot(
  ggplotly(plottpt_19_java14, tooltip = "label") %>%
    layout(
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
  ...
  ggplotly(plottpt_23_java14, tooltip = "label") %>%
    layout(
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
...
# Data Ekspor
e24v <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2024_EXIM/e24t1v.xlsx")
e23v_1314 <- e24v |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  mutate(V2023 = Vtw123 + Vtw232 + Vtw323 + Vtw423) |>
  select(Provinsi, Kbli2, Vtw124, V2023) |>
  rename(V2024 = `Vtw124`)    #2024 only Triwulan I


e24q <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2024_EXIM/e24t1q.xlsx")
e23q_1314 <- e24q |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  mutate(Q2023 = Qtw123 + Qtw232 + Qtw323 + Qtw423) |>
  select(Provinsi, Kbli2, Qtw124, Q2023) |>
  rename(Q2024 = `Qtw124`)    #2024 only Triwulan I

e23v <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2023_EXIM/e23t4v.xlsx")
e22v_1314 <- e23v |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  select(Provinsi, Kbli2, V2022)

e23q <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2023_EXIM/e23t4q.xlsx")
e22q_1314 <- e23q |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  select(Provinsi, Kbli2, Q2022)

e22v <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2022_EXIM/230123 - Nilai Ekspor TW IV 2022.xlsx")
e21v_1314 <- e22v |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  rename(V2021 = `2021`) |>
  select(Provinsi, Kbli2, V2021)

e22q <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2022_EXIM/230123 - Volume Ekspor TW IV 2022.xlsx")
e21q_1314 <- e22q |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  rename(Q2021 = `2021`) |>
  select(Provinsi, Kbli2, Q2021)

e21v <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2021_EXIM/260122_Volume Ekspor per Provinsi dan Nasional (sampai tw 4 2021).xlsx", skip = 1)
e20v_1314 <- e21v |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  rename(V2020 = `2020`) |>
  select(Provinsi, Kbli2, V2020)

e21q <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2021_EXIM/260122_Nilai Ekspor per Provinsi dan Nasional (sampai tw 4 2021).xlsx", skip = 1)
e20q_1314 <- e21q |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  rename(Q2020 = `2020`) |>
  select(Provinsi, Kbli2, Q2020)

e20v <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2020_EXIM/eks20v.xlsx", skip = 1)
e19v_1314 <- e20v |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  rename(V2019 = `Vtot19`) |>
  select(Provinsi, Kbli2, V2019)

e20q <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2020_EXIM/eks20q.xlsx", skip = 1)
e19q_1314 <- e20q |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  rename(Q2019 = `Qtot19`) |>
  select(Provinsi, Kbli2, Q2019)

e19v <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2019_EXIM/eksporv2019t4.xlsx", skip = 2)
e18v_1314 <- e19v |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  rename(V2018 = `Vtot18`) |>
  select(Provinsi, Kbli2, V2018)

e19q <- read_xlsx("D:/##AYUPA DESKTOP 2/DATA INDUSTRI/Eksim/2019_EXIM/eksporq2019t4.xlsx", skip = 2)
e18q_1314 <- e19q |> 
  filter(Provinsi == "Indonesia") |>
  filter(Kbli2 == 13 | Kbli2 == 14) |>
  rename(Q2018 = `Qtot18`) |>
  select(Provinsi, Kbli2, Q2018)


ekspor <- e23v_1314 %>%
  full_join(e23q_1314, by = c("Provinsi", "Kbli2")) %>%
  full_join(e22v_1314, by = c("Provinsi", "Kbli2")) %>%
  full_join(e22q_1314, by = c("Provinsi", "Kbli2")) %>%
  full_join(e21v_1314, by = c("Provinsi", "Kbli2")) %>%
  full_join(e21q_1314, by = c("Provinsi", "Kbli2")) %>%
  full_join(e20v_1314, by = c("Provinsi", "Kbli2")) %>%
  full_join(e20q_1314, by = c("Provinsi", "Kbli2")) %>%
  full_join(e19v_1314, by = c("Provinsi", "Kbli2")) %>%
  full_join(e19q_1314, by = c("Provinsi", "Kbli2"))

ekspor <- ekspor %>%
  pivot_longer(
    cols = starts_with("V") | starts_with("Q"),
  ...
  mutate(val = val / 1000,        # in thousand USD
         vol = vol / 1000) %>%    # in thousand kg
  mutate(Kbli_nama = case_when(
    Kbli2 == 13 ~ "Tekstil",
    Kbli2 == 14 ~ "Pakaian Jadi"
  ))

# Add info label
Statistik <- paste0(
  "\nTahun: ", ekspor$year,
  "\nIndustri: ", ekspor$Kbli_nama,
  "\nNilai (000 USD): ", scales::comma(ekspor$val, big.mark = "."),
  "\nVolume (000 kg): ", scales::comma(ekspor$vol, big.mark = ".")
)
ekspor$Statistik <- Statistik


valplot <- ggplot(ekspor, aes(x = year, y = val, color = Kbli_nama)) +
  geom_line() +
  geom_point(aes(text = Statistik)) +  
  labs(title = "Tren Nilai Ekspor, 2019 - Triwulan I 2024",
       x = "",
       y = "Export Value in 000 USD",
       color = "Industri",
       caption = "Catatan: 2024 hanya sampai triwulan 1 | Sumber: Statistik Ekspor") +
  theme_pander() +
  scale_fill_economist() +
  scale_y_continuous(labels = scales::number_format(scale = 1e-3, big.mark = ".", accuracy = 0.001)) 


ggplotly(valplot, tooltip = c("text")) %>%
  layout(
    annotations = list(
      x = 1, y = -0.09,
      xref = "paper", yref = "paper",
      text = "Catatan: 2024 hanya sampai triwulan 1 | Sumber: Statistik Ekspor",
      showarrow = FALSE,
      font = list(size = 13)
    ))
