...
```{r, warning=FALSE}
#TAMBAHKAN ANALISIS TERKAIT DAMPAK COVID!!!
```
...
## Pengeluaran untuk upah dan input

```{r, warning=FALSE}
#KASIH WARNING INPUT termasuk pengeluaran untuk upah!!
#Lalu edit deskripsi
```
...
Total pengeluaran untuk upah pekerja dan input produksi industri TPT berfluktuasi sepanjang 2016-2021. Total pengeluaran upah dan input industri TPT tertinggi terjadi pada tahun 2019, di mana nilainya mencapai _`r input1314_2019`_. Pada tahun 2021, total pengeluaran upah dan input pada _industri tekstil_ mengalami penurunan dari beberapa tahun sebelumnya menjadi _`r input13_2021`_. Sebaliknya, total pengeluaran upah dan input _industri pakaian jadi_ meningkat menjadi _`r input14_2021`_ di tahun 2021.
...
ggplot(total_input, aes(x = factor(year), y = total, fill = factor(name_isic2))) +
  ...
  geom_text(aes(label = scales::number_format(scale = 1e-3, big.mark = ".", accuracy = 0.001)(total), 
                y = total, group = name_isic2),
            position = position_stack(vjust = 0.5), color = "white") +  # Add labels for the sum of labor
  labs(title = "Total Pengeluaran untuk Upah dan Input",
       x = "",
       y = "miliar Rupiah",
       fill = "Industri",
       caption = "Sumber: Sensus Ekonomi (2016), STPIM (2017-2021)") +
