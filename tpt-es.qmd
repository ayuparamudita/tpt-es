...
# Data 2016
shp_2016_java <- shp %>% 
  left_join(num_by_year[[1]], by = c("kode" = "prov")) %>%
  mutate(tekstil    = ifelse(tekstil    == 0, NA, tekstil)) %>%
  mutate(pakaian    = ifelse(pakaian    == 0, NA, pakaian)) %>%
  mutate(total_tpt  = ifelse(total_tpt  == 0, NA, total_tpt)) %>%
...
# Add info label
Statistik2016 <- paste0(
  "\nProvinsi: ", shp_2016_java$Propinsi,
  "\nJumlah industri TPT: ", shp_2016_java$total_tpt,
  "\nJumlah industri tekstil: ", shp_2016_java$tekstil,
  "\nJumlah industri pakaian jadi: ", shp_2016_java$pakaian
)
shp_2016_java$Statistik2016 <- Statistik2016
...
plottpt_16_java <- ggplot(shp_2016_java) +
  geom_sf(aes(fill = total_tpt, label = Statistik2016)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2016_java$total_tpt)), name = "") +
...
plottpt_23_java <- ggplot(shp_2023_java) +
  geom_sf(aes(fill = total_tpt, label = Statistik2023)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2016_java$total_tpt)), name = "") +
  ggtitle("Sebaran Industri TPT 2016 (atas) dan 2023 (bawah)") +
  theme_void()


combined_plot_java <- subplot(
  ggplotly(plottpt_16_java, tooltip = "label") %>%
    layout(
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
      text = "Sumber: Sensus Ekonomi 2016 (atas), Updating DPA 2023 (bawah)",
      showarrow = FALSE,
      font = list(size = 10)
      )),
  ggplotly(plottpt_23_java, tooltip = "label") %>%
    ...
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
#      text = "Sumber: Sensus Ekonomi 2016, Updating DPA 2023",
      showarrow = FALSE,
      font = list(size = 10)
      )),
  nrows = 2)
...
tpt32_16 <- format(shp_2016_java$total_tpt[shp_2016_java$kode == 32], big.mark = ".")
tpt32_23 <- format(shp_2023_java$total_tpt[shp_2023_java$kode == 32], big.mark = ".")

```

Isu relokasi perusahaan TPT dari suatu wilayah ke wilayah lain di Pulau Jawa dikarenakan faktor kenaikan upah minimum tidak terlihat pada peta tren sebaran industri di atas. Provinsi Jawa Barat, contohnya, mengalami penurunan jumlah industri TPT sepanjang 2016 hingga 2023, dari `r tpt32_16` menjadi `r tpt32_23` perusahaan. Penurunan jumlah industri di Provinsi Jawa Barat tidak menunjukkan indikasi adanya relokasi usaha ke provinsi lain dengan upah minimum yang lebih rendah, seperti Provinsi Jawa Tengah, sebab semua provinsi di Pulau Jawa, kecuali DI Yogyakarta, tercatat mengalami penurunan jumlah perusahaan industri TPT sepanjang 2016-2023.



```{r, warning=FALSE, include=FALSE}
# Tekstil
plottpt_16_java13 <- ggplot(shp_2016_java) +
  geom_sf(aes(fill = tekstil, label = Statistik2016)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2016_java$total_tpt)), name = "") +
#  ggtitle("Sebaran Industri TPT 2016") +
  theme_void()
...
plottpt_23_java13 <- ggplot(shp_2023_java) +
  ...
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2016_java$total_tpt)), name = "") +
  ggtitle("Sebaran Industri Tekstil 2016 (atas) dan 2023 (bawah)") +
  theme_void()


combined_plot_java13 <- subplot(
  ggplotly(plottpt_16_java13, tooltip = "label") %>%
    layout(
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
...
# Pakaian Jadi
plottpt_16_java14 <- ggplot(shp_2016_java) +
  geom_sf(aes(fill = pakaian, label = Statistik2016)) +
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2016_java$total_tpt)), name = "") +
#  ggtitle("Sebaran Industri TPT 2016") +
  theme_void()
...
plottpt_23_java14 <- ggplot(shp_2023_java) +
  ...
  scale_fill_viridis_c(limits = range(c(shp_2023_java$total_tpt, shp_2016_java$total_tpt)), name = "") +
  ggtitle("Sebaran Industri Pakaian Jadi 2016 (atas) dan 2023 (bawah)") +
  theme_void()


combined_plot_java14 <- subplot(
  ggplotly(plottpt_16_java14, tooltip = "label") %>%
    layout(
    annotations = list(
      x = 1, y = -0.05,
      xref = "paper", yref = "paper",
